...
for (v of values) {
  ...
  if (!Array.isArray(valArr[v.indicatorId][v.year])) valArr[v.indicatorId][v.year]=[];
...
for (let year=2009; year<2019;year++) {
  sitesAtYear[year]=[];
  for (r of regions) {
    let say = sites.reduce((cnt,s)=>cnt+=(s.year<=year && s.osmId==r.osmId)?1:0,0);
...
for (let year=2009; year<2019;year++) {
  ...
  for (r of regions) {
    deaths1Y_PerBirth[year][r.osmId]=valArr[1][year][r.osmId]/valArr[0][year][r.osmId];
    deaths7D_PerBirth[year][r.osmId]=valArr[4][year][r.osmId]/valArr[0][year][r.osmId];
    deadborns_PerBirth[year][r.osmId]=valArr[3][year][r.osmId]/valArr[0][year][r.osmId];
    perinatal_PerBirth[year][r.osmId]=valArr[2][year][r.osmId]/valArr[0][year][r.osmId];
  }
}
...
function corr(d) {
  ...
  for (r of regions) {
    ...
    let x2_sum=0, y2_sum=0, xy_sum=0;
    if(d_cnt && say_sum) {
      for (let year=2009; year<2019;year++) {
